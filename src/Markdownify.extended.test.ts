...
describe("Markdownify Extended", () => {
      ...
      const mockFetch = mock((_url: string, _init?: any) => {
        ...
        if (callCount === 1) {
          return Promise.resolve({
            status: 302,
            headers: { get: (name: string) => name === "location" ? "/final" : null },
          } as any);
        }
        return Promise.resolve({
          status: 200,
  ...
  describe("get() method — extended", () => {
    ...
    test("get rejects non-markdown files with clear error", async () => {
      ...
      try {
        await expect(
          Markdownify.get({ filePath: tmpFile }),
        ).rejects.toThrow("Required file is not a Markdown file.");
      } finally {
        if (fs.existsSync(tmpFile)) fs.unlinkSync(tmpFile);
      }
    });
    ...
    test("get respects MD_ALLOWED_PATHS restriction", async () => {
      ...
      try {
        ...
        const shouldThrow =
          !path.resolve(os.tmpdir()).startsWith(path.resolve("/tmp/allowed"));
        if (shouldThrow) {
          await expect(
            Markdownify.get({ filePath: tmpFile }),
  ...
  describe("fromRepo edge cases", () => {
    test("fromRepo rejects flag injection via --help", async () => {
      await expect(
        Markdownify.fromRepo({ repoUrl: "--help" }),
  ...
  describe("Error message quality", () => {
    test("toMarkdown error wraps original error message", async () => {
      await expect(
        Markdownify.toMarkdown({}),
      ).rejects.toThrow("Error processing to Markdown: Either filePath or url must be provided");
    });
    ...
    test("get error message is clear about non-markdown files", async () => {
      ...
      const pdfPath = path.join(sampleDataDir, "test.pdf");
      await expect(
        Markdownify.get({ filePath: pdfPath }),
      ).rejects.toThrow("Required file is not a Markdown file.");
    });
  });
});
